...
class RandomForestMSE:
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        for i in range(self.loop_estimators):
            ...
            self.res += self.trees[-1].predict(X[:,self.feat_ids_by_tree[-1]])
            self.history.append(np.power(self.res / len(self.trees) - y, 2).mean())
...
class GradientBoostingMSE:
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        if not self.reFit:
            ...
            self.history = [np.power(self.res - y, 2).mean()]
        ...
        for i in range(self.loop_estimators):
            ...
            self.history.append(np.power(self.res - y, 2).mean())
